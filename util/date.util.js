...
exports.isDateEquals = (dateString1, dateString2) => {
    let date1 = moment(this.getDateFromUTCString(dateString1)).format('YYYY-MM-DD')
    let date2 = moment(this.getDateFromUTCString(dateString2)).format('YYYY-MM-DD')
...
exports.getDateDifferenceInDays = (dateString1, dateString2) => {
    let date1 = moment(new Date(dateString1)).format('YYYY-MM-DD')
    let date2 = moment(new Date(dateString2)).format('YYYY-MM-DD')
    return date1.diff(date2, 'days')
}

exports.weeksDiff = (d1, d2) => {
    let date1 = moment(new Date(d1)).format('YYYY-MM-DD')
    let date2 = moment(new Date(d2)).format('YYYY-MM-DD')
...
exports.getDayOfWeek = (date) => {
    if (date)
        return moment(new Date(date)).format('YYYY-MM-DD').day()
    return moment().format('YYYY-MM-DD').day()
}

exports.getQuarter = (dateString) => {
    if (dateString)
        return moment(new Date(dateString)).format('YYYY-MM-DD').quarter()
...
exports.getDaysInMonth = (date) => {
    if (date)
        return moment(new Date(date)).format('YYYY-MM').daysInMonth()
